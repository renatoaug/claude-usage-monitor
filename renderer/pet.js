...
(function buildScene() {
  ...
  [[100, 16], [60, 22], [150, 100], [196, 56], [205, 98], [128, 26], [182, 22], [202, 14]]
    .forEach(([x, y]) => star(x, y));
})();
...
function spawnCoin() {
  ...
  else if (side === 1) { x = W + 10; y = Math.random() * H * 0.7; }
  else if (side === 2) { x = Math.random() * W; y = H + 10; }
  else { x = -10; y = Math.random() * H * 0.7; }
  ...
  const anim = coin.animate(
    [
      ...
      { transform: `translate(${mxo}px,${myo}px) scale(1) rotate(${(rot * 0.6).toFixed(0)}deg)`, opacity: 1, offset: 0.55 },
      { transform: `translate(${dx}px,${dy}px) scale(0.2) rotate(${rot.toFixed(0)}deg)`, opacity: 0 },
    ],
  ...
  for (let i = 0; i < n; i++) {
    ...
    const a = h.animate(
      [
        ...
        { transform: `translate(${(dx * 0.5).toFixed(1)}px, -12px) scale(1.3)`, opacity: 1, offset: 0.3 },
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const a = c.animate(
      [
        ...
        { transform: `translate(${(Math.cos(ang) * d).toFixed(1)}px, ${(Math.sin(ang) * d - 4).toFixed(1)}px) scale(0.2)`, opacity: 0 },
      ],
...
function spawnConfetti(i) {
  ...
  const a = p.animate(
    [
      ...
      { transform: `translate(${ux.toFixed(0)}px, ${uy.toFixed(0)}px) rotate(${(rot * 0.4).toFixed(0)}deg) scale(1)`, opacity: 1, offset: 0.4 },
      { transform: `translate(${(ux * 1.4).toFixed(0)}px, ${fallY.toFixed(0)}px) rotate(${rot.toFixed(0)}deg) scale(0.9)`, opacity: 0 },
    ],
...
function render(d) {
  ...
  const word =
    st === 'working' ? 'working' : st === 'sleeping' ? 'sleeping' : st === 'stressed' ? 'on fire' : 'idle';
...
window.addEventListener('mousemove', (e) => {
  ...
  eyesG.setAttribute('transform', `translate(${((dx / len) * 3).toFixed(2)} ${((dy / len) * 2).toFixed(2)})`);
});
